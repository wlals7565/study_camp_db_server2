...
export class SpaceMembersService {
  ...
  async addMemberInSpace(addMemberInSpaceDto: AddMemberInSpaceDto) {
    ...
    let existingMember = await this.spaceMemberRepository.findOne({
      where: {
    ...
    let member = this.spaceMemberRepository.create({
      user_id: addMemberInSpaceDto.userId,
      space_id: addMemberInSpaceDto.spaceId,
  ...
  async findUserById(userId) {
    try {
      let exUser = this.spaceMemberRepository.findOneBy({ user_id: userId });
      return exUser;
    } catch (error) {
      throw new InternalServerErrorException('서버 오류 발생');
    }
